...
class DbusShellyRpcService:
  def __init__(self, servicename, paths, productname='', connection='Shelly JSON-RPC 2.0 service'):
  ...
  def _getShellyFirmwareVersion(self):
    config_data = self._getShellyConfig()
    fw_version = config_data['sys']['device']['fw_id']
    if not fw_version:
        return 'N/A' 
    return fw_version

  def _getShellySerial(self):
    meter_data = self._getShellyStatus()  
  ...
  def _getShellyStatusUrl(self):
    ...
    if accessType == 'OnPremise':
        useAuthentication = config['ONPREMISE']['UseAutentication']
        if useAuthentication != 'true':
            URL = "http://%s/rpc/Shelly.GetStatus" % (config['ONPREMISE']['Host'])
        else:
            # https://shelly-api-docs.shelly.cloud/gen2/General/Authentication#setting-authentication-credentials-for-a-device
            raise ValueError("OnPremise Authentication is not supported yet")
    else:
  ...
  def _getShellyConfigUrl(self):
    ...
    if accessType == 'OnPremise':
        useAuthentication = config['ONPREMISE']['UseAutentication']
        if useAuthentication != 'true':
            URL = "http://%s/rpc/Shelly.GetConfig" % (config['ONPREMISE']['Host'])
        else:
            # https://shelly-api-docs.shelly.cloud/gen2/General/Authentication#setting-authentication-credentials-for-a-device
            raise ValueError("OnPremise Authentication is not supported yet")
    else:
  ...
  def _getShellyStatus(self):
    URL = self._getShellyStatusUrl()
    return self._callRpcFunction(URL)
 
  def _getShellyConfig(self):
    URL = self._getShellyConfigUrl()
    return self._callRpcFunction(URL)

  def _callRpcFunction(self, URL):
...
def main():
  ...
  try:
      ...
      pvac_output = DbusShellyRpcService(
